...
async function extractViaPiped(videoId) {
  for (const instance of PIPED_INSTANCES) {
    // Try all instances — both Piped and Invidious have stream/subtitle endpoints
    try {
      const controller = new AbortController();
      ...
      if (!res.ok) {
        console.log(`[RSE] Piped ${instance} returned ${res.status} for ${videoId}`);
        continue;
      }
      const data = await res.json();

      // Piped/Invidious returns subtitles array with url field
      ...
      // Fetch the actual subtitle content with proper timeout
      const subController = new AbortController();
      const subTimer = setTimeout(() => subController.abort(), 10000);
      const subRes = await fetch(enSubs.url, { signal: subController.signal });
      ...
      console.warn(`[RSE] Piped instance ${instance} failed for ${videoId}: ${err.message}`);
      continue;
    }
  }
