 ...
 * CORRECT approach — goes through server's in-memory DB, no isolation issues.
 *
 * ⚠️  Server (npm run dev) must be running first.
 *
 * Usage:
 *   node scripts/trigger-scout.js                    # auto-rotation (next market)
 *   node scripts/trigger-scout.js "Phoenix, AZ"      # specific market
 *   node scripts/trigger-scout.js "Texas"            # whole state
 */
'use strict';
...
async function main() {
  ...
  // Auth response: { token: { token: "...", expiresIn: ... } } or { token: "..." }
  ...
  const msg = region
    ? JSON.stringify({ region, trade: 'GC', limit: 8 })
    : '{}';

  console.log(`🚀 Triggering jake-lead-scout${region ? ` for "${region}"` : ' (auto-rotation)'}...`);

  // Create pending run (agent accepts name as id)
  const createResp = await request('POST', '/api/agents/jake-lead-scout/run', {
    message: msg, json: true,
  }, token);

  if (!createResp.run?.id) { console.error('Create run failed:', JSON.stringify(createResp)); process.exit(1); }

  const runId = createResp.run.id;
  console.log(`📋 Run ID: ${runId}`);
  console.log(`⏳ Executing (2-5 min)...\n`);

  // Confirm + execute (synchronous — waits for agent to finish)
  ...
  if (r.result_data) {
    try { console.log('\n' + JSON.parse(r.result_data).outputText); } catch {}
  }
